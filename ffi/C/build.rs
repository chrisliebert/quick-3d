...
fn copy_quick3d_shared_library_files() -> std::io::Result<()> {
	...
	try!(fs::copy(Path::new("../../target/debug/quick3d.dll"), Path::new("src/quick3d.dll")));
	try!(fs::copy(Path::new("../../target/debug/quick3d.dll.exp"), Path::new("src/quick3d.dll.exp")));
	try!(fs::copy(Path::new("../../target/debug/quick3d.dll.lib"), Path::new("src/quick3d.dll.lib")));
	try!(fs::copy(Path::new("../../target/debug/quick3d.pdb"), Path::new("src/quick3d.pdb")));
	Ok(())
}
...
fn copy_quick3d_shared_library_files() -> std::io::Result<()> {
	...
	try!(fs::copy(Path::new("../../target/debug/libquick3d.so"), Path::new("src/libquick3d.so")));
	Ok(())
}
